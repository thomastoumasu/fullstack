...
const getCountryNames = () => { // return all country names
  ...
  console.log(`--countryService: fetching all country names from ${url}`)
  return axios
    .get(url)
    .then(response => {
      const names = response.data.map(e => e.name.common)
      console.log('--countryService: got response from server: ', names)
      return names
    })
}
...
const getCountryInfo = (country) => { // return country info for one country
  ...
  console.log(`--countryService: fetching country info for ${country} from ${url}`)
  return axios
    .get(url)
